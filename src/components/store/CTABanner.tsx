const WHATSAPP_NUMBER = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER ?? "5511999999999";

export default function CTABanner() {
  const href = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(
    "Olá! Quero começar minha jornada de saúde com a Naturalli. Pode me ajudar?"
  )}`;

  return (
    <section className="py-20 bg-gradient-to-r from-brand-red-dark via-brand-red to-brand-red-dark">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <span className="inline-block px-4 py-1 text-xs font-semibold tracking-widest text-brand-gold border border-brand-gold/40 rounded-full mb-6 uppercase">
          Comece hoje
        </span>

        <h2 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4 max-w-2xl mx-auto">
          Comece sua jornada de saúde hoje
        </h2>

        <p className="text-white/80 text-lg mb-10 max-w-xl mx-auto">
          Fale com nosso especialista pelo WhatsApp e receba uma consultoria gratuita
          para montar o protocolo ideal para você.
        </p>

        <a
          href={href}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-3 px-10 py-5 bg-brand-gold text-black font-bold text-lg rounded-xl hover:bg-brand-gold-light transition-all duration-200 shadow-2xl hover:shadow-brand-gold/40 hover:-translate-y-0.5 animate-pulse-gold"
        >
          <WhatsAppIcon />
          Falar com Especialista no WhatsApp
        </a>

        <p className="mt-6 text-white/50 text-sm">
          Atendimento de segunda a sábado, das 8h às 20h.
        </p>
      </div>
    </section>
  );
}

function WhatsAppIcon() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="currentColor"
      className="w-6 h-6"
    >
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" />
      <path d="M12 0C5.373 0 0 5.373 0 12c0 2.126.558 4.122 1.528 5.855L.057 23.75a.75.75 0 0 0 .944.943l5.925-1.47A11.952 11.952 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.75a9.726 9.726 0 0 1-4.953-1.352l-.354-.212-3.664.91.926-3.583-.23-.368A9.724 9.724 0 0 1 2.25 12C2.25 6.615 6.615 2.25 12 2.25S21.75 6.615 21.75 12 17.385 21.75 12 21.75z" />
    </svg>
  );
}
